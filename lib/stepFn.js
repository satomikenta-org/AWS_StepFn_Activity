...
class StepFnService {
  ...
  async sendTaskToken (token) {
    const output = JSON.stringify({ number: 2 });
    const params = { taskToken: token, output };
    return await this.StepFunctions.sendTaskSuccess(params).promise();
  }
